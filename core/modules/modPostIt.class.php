...
//        );
	...
	public function init($options = '')
	{
		...
		// Document templates
		$moduledir = dol_sanitizeFileName('postit');
		$myTmpObjects = array();
		$myTmpObjects['PostIt'] = array('includerefgeneration'=>0, 'includedocgeneration'=>0);

		foreach ($myTmpObjects as $myTmpObjectKey => $myTmpObjectArray) {
			if ($myTmpObjectKey == 'PostIt') {
				continue;
			}
			if ($myTmpObjectArray['includerefgeneration']) {
				$src = DOL_DOCUMENT_ROOT.'/install/doctemplates/'.$moduledir.'/template_postits.odt';
				$dirodt = DOL_DATA_ROOT.'/doctemplates/'.$moduledir;
				$dest = $dirodt.'/template_postits.odt';
